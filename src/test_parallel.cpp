...
vector<int> THREADS_NUM_LIST = {1,2};
vector<float> COMPUTATION_TIME_LIST = {0};
...
void *run_insert(void *i)
{
   ...
   // printf("%dcount",count);
    ...
    {
        int element = start[i];
        avl_insert(root, element);
        usleep(sleep_time);
       // printf("[RUN] finish inserting element %d\n", element);
    }
    //pthread_exit(NULL);
    return NULL;
}
...
int run_multi_thread_insert(int thread_count)
{
    ...
    {
	//std::cout << tid[i];
        int result = pthread_create(&tid[i], NULL, run_insert, (void *)(i + 1));
	if (0 != result)
  	{
    		std::cout<< "error";
  	}
...
int run_multi_thread_remove(int thread_count)
{
    ...
    for (int i = 0; i < thread_count; i++)
    {
        int res = pthread_create(&tid[i], NULL, run_remove, (void *)(i + 1));
	if (0 != res)
  	{
    		std::cout<< "error";
  	}
